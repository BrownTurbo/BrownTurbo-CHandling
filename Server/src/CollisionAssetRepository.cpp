...
namespace scv::server::collision
{
    ...
    bool
    CollisionAssetRepository::Initialize(
    ...
    bool
    CollisionAssetRepository::IsValidNameByte(
        std::uint8_t c)
    {
        return
            c == 0 ||
    ...
    bool
    CollisionAssetRepository::ReadName(
    ...
    bool
    CollisionAssetRepository::IsZeroTail(
        const std::uint8_t* data,
        std::size_t size)
    {
        if (!data)
    ...
    bool
    CollisionAssetRepository::RangeWithin(
        std::size_t offset,
        std::size_t size,
        std::size_t total)
    {
    ...
    bool
    CollisionAssetRepository::LoadWholeFile(
    ...
    bool
    CollisionAssetRepository::ComputeSha256(
        const std::filesystem::path& path,
        Hash256& outHash,
        std::string& outError) const
    {
        ...
        {
            outError =
                "Unable to open COL for SHA-256.";
        ...
        {
            outError =
                "BCrypt SHA-256 provider failed.";
                ...
                0) < 0)
        {
            ...
            outError =
                "BCrypt object-size query failed.";
                ...
                0) < 0)
        {
            ...
            outError =
                "BCrypt digest-size query failed.";
        ...
        std::string& outError) const
    {
        ...
        outMetadata.fileSize =
            bytes.size();

        outMetadata.recordCount =
            static_cast<std::uint32_t>(
                outRecords.size());

        outMetadata.firstVersion =
            outRecords.front().version;

        outMetadata.multipleRecords =
            outRecords.size() > 1;

        outMetadata.mixedVersions =
            std::any_of(
                outRecords.begin() + 1,
                outRecords.end(),
    ...
    bool
    CollisionAssetRepository::DiscoverModelId(
        const std::filesystem::path&
            directory,
    ...
    bool
    CollisionAssetRepository::Reload(
        CustomModelId customModelId,
        std::string& outError)
    {
        const auto path =
