...
public:
    ...
    static void DestroyCustomModel(uint32_t customId) {
        ...
        if (it == s_customModels.end()) return;
    ...
    static void ClearAllCustomModels() {
        for (auto& [id, pInfo] : s_customModels) {
            if (pInfo) {
                if (pInfo->m_pRwClump) RpClumpDestroy(pInfo->m_pRwClump);
                if (pInfo->m_nTxdIndex != -1) CTxdStore::RemoveTxdSlot(pInfo->m_nTxdIndex);
                delete pInfo;
            }
