...
private:
    ...
    void WorkerLoop() {
        while (!m_stopPool) {
            AssetDownloadTask task;
            {
                ...
                m_cv.wait(lock, [this] { return m_stopPool || !m_taskQueue.empty(); });

                if (m_stopPool && m_taskQueue.empty()) return;
    ...
    void ExecuteTask(const AssetDownloadTask& task) {
        ...
        if (_stricmp(result.calculatedHash.c_str(), task.expectedSha256.c_str()) != 0) {
            ...
            result.errorMessage = "Downloaded file hash (" + result.calculatedHash + 
                                  ") does not match expected hash (" + task.expectedSha256 + ").";
            if (task.onComplete) task.onComplete(result);
            return;
        }
